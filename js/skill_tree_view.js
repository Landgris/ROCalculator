...
(function () {
    ...
    Vue.component('skill-tree-dialog', {
            ...
            buildDetail: function (jobKey, skid) {
                ...
                for (var lv = 1; lv <= info.maxLv; lv++) {
                    levelsInfo.push({
                        ...
                        singlePostDelay: delay.singlePostDelay ? delay.singlePostDelay[lv - 1] : undefined,
                        globalPostDelay: delay.globalPostDelay ? delay.globalPostDelay[lv - 1] : undefined,
                    });
                }
                return {
                    skid: skid,
        ...
        template: [
            ...
            '          <div class="skill-tree-lv-table-wrap">',
            '          <table class="skill-tree-lv-table">',
            '            <tr><th>Lv</th><th>SP</th><th v-if="hasDelayData">{{ t(\'ui.skilltree.fixedcast\', \'固定詠唱\') }}</th><th v-if="hasDelayData">{{ t(\'ui.skilltree.variablecast\', \'變動詠唱\') }}</th><th v-if="hasDelayData">{{ t(\'ui.skill.independentdelay\', \'獨立延遲\') }}</th><th v-if="hasDelayData">{{ t(\'ui.skill.globaldelay\', \'共通延遲\') }}</th></tr>',
            ...
            '              <td v-if="hasDelayData">{{ formatDelaySeconds(li.singlePostDelay) }}</td>',
            '              <td v-if="hasDelayData">{{ formatDelaySeconds(li.globalPostDelay) }}</td>',
            '            </tr>',
            '          </table>',
            '          </div>',
            '        </div>',
